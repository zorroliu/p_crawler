...
class CustLogger:
    def __init__(self, name, log_path: str = None):
        ...
        self._log_format = "<green>{time:YYYY-MM-DD HH:mm:ss}</green> | <level>{level: <8}</level> | <cyan>{" \
    ...
    def log(self, message, level: LogLevel = LogLevel.INFO):
        self._logger = logger.bind(name=self._bind_name)

        self._logger.remove()
        self._logger.add(self._log_path, rotation="00:00", format=self._log_format, encoding="utf-8")
        self._logger.add(sys.stdout, format=self._log_format)
